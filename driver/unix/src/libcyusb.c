...
//	printf("Library: Total number of devices detected = %d\n",numdev);
//	printf("Library: -------------------------------------\n");
...
//		   printf("Device of interest found. Bus = %d, DevAddr = %d, VID = %04x, PID = %04x\n",
//					cydev[nid].busnum, cydev[nid].devaddr, cydev[nid].vid, cydev[nid].pid );
		   ++nid;
		}
	}
	return nid;
...
{
	cyusb_device *tdev = libusb_get_device(h);
	return ( libusb_get_config_descriptor_by_value(tdev, bConfigurationValue, (struct libusb_config_descriptor **)config) );
}
...
int cyusb_get_descriptor(cyusb_handle *h, unsigned char desc_type, unsigned char desc_index, unsigned char *data, int len)
{
...
int cyusb_control_transfer(cyusb_handle *h, unsigned char bmRequestType, unsigned char bRequest, unsigned short wValue, 
